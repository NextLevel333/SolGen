...
import { CONFIG, PerformanceMode, PERFORMANCE_CONFIG } from '../config/constants';
...
import { formatDuration, estimateTime } from '../utils/format';
...
export const VanityContractForm: React.FC = () => {
  ...
  // Helper function to get safe worker count (same as VanityGenerator)
  ...
  // Aggregate progress from all workers (same as VanityGenerator)
  ...
  const handlePaymentComplete = async () => {
    ...
    try {
      ...
      // Move to performance mode selection step
  ...
  const handleCancelAction = (action: 'retry' | 'change') => {
    if (action === 'retry') {
      // Retry with same settings - go back to mode selection
      setCurrentStep('mode-select');
    } else {
      // Change characters - go back to form
      setCurrentStep('form');
    }
  };
  ...
  // Render performance mode selection step
    ...
    return (
      <div className="solana-card p-8 space-y-6">
        <div className="text-center">
          ...
          <p className="text-gray-400">
            Choose how fast you want to generate your custom contract address
          </p>
        </div>
    ...
    return (
      <div className="solana-card p-8 space-y-6">
        <div className="text-center">
          ...
          <p className="text-gray-400">
            Custom contract address generation in <span className="capitalize font-semibold text-solana-purple">{performanceMode}</span> mode
          </p>
        ...
        <div className="space-y-4">
          <div className="bg-gray-700/50 p-6 rounded-lg space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="font-mono font-semibold">
                {formData.vanityPosition === 'prefix' ? `${formData.vanityCharacters}...` : `...${formData.vanityCharacters}`}
              </span>
            </div>
